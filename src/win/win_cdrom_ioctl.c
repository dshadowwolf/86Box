...
static cdrom_ops_t ioctl_atapi;
...
get_track_nr(uint32_t pos)
{
    ...
    for (c = toc.FirstTrack; c < toc.LastTrack; c++) {
        uint32_t track_address = toc.TrackData[c].Address[3] + (toc.TrackData[c].Address[2] * 75) + (toc.TrackData[c].Address[1] * 75 * 60);
...
static void
ioctl_playaudio(uint32_t pos, uint32_t len, int ismsf)
{
    if (!cdrom_drive)
        return;
...
ioctl_ready(void)
{
    ...
    if ((ltoc.TrackData[ltoc.LastTrack].Address[1] != toc.TrackData[toc.LastTrack].Address[1]) || (ltoc.TrackData[ltoc.LastTrack].Address[2] != toc.TrackData[toc.LastTrack].Address[2]) || (ltoc.TrackData[ltoc.LastTrack].Address[3] != toc.TrackData[toc.LastTrack].Address[3]) || !tocvalid || (cdrom_drive != old_cdrom_drive)) {
...
static int
ioctl_get_last_block(unsigned char starttrack, int msf, int maxlen, int single)
{
    long      size;
    int       c;
    CDROM_TOC lbtoc;
...
ioctl_medium_changed(void)
{
    ...
    if ((ltoc.TrackData[ltoc.LastTrack].Address[1] != toc.TrackData[toc.LastTrack].Address[1]) || (ltoc.TrackData[ltoc.LastTrack].Address[2] != toc.TrackData[toc.LastTrack].Address[2]) || (ltoc.TrackData[ltoc.LastTrack].Address[3] != toc.TrackData[toc.LastTrack].Address[3])) {
        ioctl_cd_state = CD_STOPPED;
        toc            = ltoc;
        cdrom_capacity = ioctl_get_last_block(0, 0, 4096, 0);
...
ioctl_getcurrentsubchannel(uint8_t *b, int msf)
{
    ...
    if (ioctl_cd_state == CD_PLAYING || ioctl_cd_state == CD_PAUSED) {
        ...
        uint32_t track_address = toc.TrackData[track].Address[3] + (toc.TrackData[track].Address[2] * 75) + (toc.TrackData[track].Address[1] * 75 * 60);
...
static int
ioctl_readsector(uint8_t *b, int sector, int count)
{
    LARGE_INTEGER pos;
    BOOL          status;
    long          size = 0;
...
is_track_audio(uint32_t pos)
{
    ...
    for (c = 0; toc.TrackData[c].TrackNumber != 0xaa; c++) {
        ...
        if (toc.TrackData[c].TrackNumber >= toc.FirstTrack && toc.TrackData[c].TrackNumber <= toc.LastTrack && track_address >= pos) {
...
static int
ioctl_is_track_audio(uint32_t pos, int ismsf)
{
    if (ismsf) {
        int m = (pos >> 16) & 0xff;
        int s = (pos >> 8) & 0xff;
...
static void
ioctl_readsector_raw(uint8_t *b, int sector)
{
    long size;
...
static int
ioctl_readtoc(unsigned char *b, unsigned char starttrack, int msf, int maxlen, int single)
{
    int      len = 4;
    long     size;
    int      c, d;
...
static int
ioctl_readtoc_session(unsigned char *b, int msf, int maxlen)
{
    int                    len = 4;
    int                    size;
    uint32_t               temp;
...
static int
ioctl_readtoc_raw(unsigned char *b, int maxlen)
{
    int                     len = 4;
    int                     size;
    int                     i;
